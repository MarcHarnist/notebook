...
<div class="m-2 p-2 border border-primary rounded">
	<p>Etes-vous sur de vouloir supprimer le chèque  <strong>n°<?=$database['id']?> </strong> du  <strong><?=$database['date'];?> </strong> à l'ordre de  <strong>"<?=$database['ordre'];?>"</strong> d'un montant de <strong><?=$database['montant']?></strong> €?</p>
</div>

<article>
  <header class="row bg-light p-3 ">
    <h2 class="row ml-0 text-muted">PANORAMA DE LA BNP</h2>
  </header>   
  ...
  <div class="col-sm-12 mt-3">
	<p>	  
		...
		Total général = crédit + à venir + solde;
	</p>
...
<!-- ************************ AFFICHAGE UPDATE ******************* -->
	<table class="table table-striped"><!-- Bootstrap class -->
	  ...
	  foreach($datas as $data){
		  ...
		  $total_general = $credit + $avenir + $solde;
		?>  
		  <tr id="<?php $ancre=$count; echo $ancre; $count++;?>">
		  
		  <!-- id du formulaire supprimés: leur duplication != valid html5 --> 
		  
		  <form method="post" action="<?= $website->page_url;?>budget-panorama-update">
		  ...
		  <td>
			<input class="right"  type="text" size="<?php echo $size;?>" name="credit" value="<?php echo $credit; ?>" />
		  </td>
		  <td>
	  ...
	  <td class="center">
		...
		<input type="submit" value="Edit" />
		  ...
		  <td class="center"><span class="center">
			<form method="post" action="<?= $website->page_url;?>budget-panorama-delete">
	...
	<table class="table table-striped">
	  ...
	  <tr>
		<form method="post" action="<?= $website->page_url;?>budget-panorama-update">
		  ...
		  <td>
			<input class="right" type="text" size="<?php echo $size;?>" name="credit"  value = "<?=$credit;?>" />
		  </td>
		  <td class="center">
			<input type="hidden" name="ancre" value="<?php echo $ancre;?>" />

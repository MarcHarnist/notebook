...
<table class="table table-striped">
	<tr>
		<th class="text-center">Priorité</th><th class="text-center">Jour</th><th>Acteur</th><th>Poste</th><th class="text-right">Entrée</th><th class="text-right">Sortie</th><th class="text-right">Cumul</th>
	</tr>
	<?php
	...
	foreach($echeancier as $donnees) {
		...
		<form method="post" action="<?= $website->page_url;?>budget-update-calcul">
			...
			<td class="text-right"><!-- entrée -->
			    <?php if($affiche_mois > 0){echo $affiche_mois; $total_entrees += $affiche_mois; echo ' €';} ?>
				</td>
			<td class="text-right"><!-- sortie -->
			    <?php if($affiche_mois <= 0) { echo $affiche_mois; $total_sorties += $affiche_mois; echo ' €';}?>
				</td>
				
				<!-- cumul -->
			<td class="<?php if($day >= date('d')) echo 'text-danger';?> text-right">
			    <?php 	echo $total_mois . "  €";
	...
	<tr class="total">
		...
		<td class="text-right"><p class="right">Total</p>
			</td>
		<td class="text-right"><p class="right"><?= $total_entrees;?>  € </p>
			</td>
		<td class="text-right"><p class="right"><?= $total_sorties;?>  € </p>
			</td>
		<td class="text-right"><p class="right"><?php echo $total_mois;?>  €</p>
			</td>
		<td class="text-right"> 
		</td>
	</tr>

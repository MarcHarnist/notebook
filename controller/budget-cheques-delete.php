...
if($confirm == "oui") {
	$database = $database->budget_cheques("delete", $_POST);
	$url= $website->page_url . 'budget-cheques#' . $database['ancre']; //$id est l'ancre de retour...
	header("Location:$url");
}
else {
	$database = $database->budget_cheques("select", $_POST);
	$id = $database['id'];
	$date = $database['date'];
	$ancre = $database['ancre'];
	$ordre = $database['ordre'];
	$montant = $database['montant'];
	$url_de_retour_en_arriere = $website->page_url . 'budget-cheques#' . $ancre;
}